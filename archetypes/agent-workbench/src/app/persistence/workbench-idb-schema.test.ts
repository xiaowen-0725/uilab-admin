...
function uniqueDbName(suffix: string): string {
  return `test-idb-v3-${suffix}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`
}
...
describe('Workbench IDB v3', () => {
  ...
  it('keeps all seven v2 store rows after upgrading to v3', async () => {
    ...
    const v3 = await openWorkbenchIdb({ name })
    expect(v3.version).toBe(3)
    for (const store of V2_STORE_NAMES) {
      expect(v3.objectStoreNames.contains(store), store).toBe(true)
    }
    expect(await getFromStore(v3, STORE_PROJECTS, 'project-keep')).toMatchObject({
      id: 'project-keep',
      name: '保留项目',
    })
    expect(await getFromStore(v3, STORE_TASKS, 'task-keep')).toMatchObject({
      id: 'task-keep',
      title: '保留任务',
    })
    expect(
      await getFromStore(v3, STORE_EVENTS, ['task-keep', 1]),
    ).toMatchObject({ eventId: 'event-keep' })
    expect(await getFromStore(v3, STORE_SNAPSHOTS, 'task-keep')).toMatchObject({
      kind: 'snapshot',
    })
    expect(await getFromStore(v3, STORE_COMMANDS, 'command-keep')).toMatchObject({
      commandId: 'command-keep',
    })
    expect(await getFromStore(v3, STORE_SESSION, 'current')).toMatchObject({
      selectedTaskId: 'task-keep',
    })
    expect(await getFromStore(v3, STORE_METADATA, 'seed')).toMatchObject({
      value: 'v2-row',
    })
    for (const store of BOARD_STORE_NAMES) {
      expect(v3.objectStoreNames.contains(store), store).toBe(true)
