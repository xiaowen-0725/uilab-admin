...
export type InteractiveClientToolName =
  (typeof INTERACTIVE_CLIENT_TOOL_NAMES)[number]

export type InteractiveArtifactToolName =
  (typeof INTERACTIVE_ALL_TOOL_NAMES)[number]

export function isInteractiveClientTool(
  name: string,
): name is InteractiveClientToolName {
  return (INTERACTIVE_CLIENT_TOOL_NAMES as readonly string[]).includes(name)
}
