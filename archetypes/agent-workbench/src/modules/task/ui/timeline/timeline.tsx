...
/** Prefer Chinese turn chrome labels; hide raw English machine status as primary. */
function chineseStatusLabel(item: TimelineItem): string {
  if (item.title && /[\u4e00-\u9fff]/.test(item.title)) {
    // Completed with duration:「已处理 22s」
    if (item.status === 'completed' && item.meta?.durationMs != null) {
      return `已处理 ${formatDurationMs(item.meta.durationMs)}`
    }
    if (item.status === 'running' && (item.title === '处理中' || item.title === '正在思考')) {
      return '正在思考'
    }
    return item.title
  }
...
/** Codex content-area: completed turn hides tool/reasoning trajectory by default. */
function TimelineTurnBlock({
  ...
  const toolish = streamItems.filter((i) =>
    ['tool-group', 'command', 'reasoning', 'plan', 'source-group'].includes(
      i.category,
    ),
  )
  const alwaysVisible = streamItems.filter(
    (i) =>
      !['tool-group', 'command', 'reasoning', 'plan', 'source-group'].includes(
        i.category,
      ),
  ...
  useEffect(() => {
    if (runActive) setToolsExpanded(true)
    ...
    <>
      {latestTerminal ? (
        <>
          <TurnHeader
            item={latestTerminal}
            canToggle={Boolean(completed && toolish.length > 0)}
      ...
      {showTools
        ? toolish.map((item) => (
            <TimelineRow
              ...
              onReject={onReject}
              onProvideInput={onProvideInput}
              forceToolCollapsed={completed || item.status === 'completed'}
            />
          ))
        : null}
      ...
      <LiveStatusLine
        status={runActive ? liveStatus : null}
        className='mt-2'
      />
    </>
  )
...
function TurnHeader({
  ...
  const label = chineseStatusLabel(item)
  const duration =
    item.meta?.durationMs != null && completed
      ? formatDurationMs(item.meta.durationMs)
      : null
  const display = duration ? `${label} ${duration}` : label
  ...
  if (canToggle && onToggleTools) {
    return (
      ...
      >
        ...
        {duration ? (
          <span className='sr-only' data-testid='timeline-run-status-badge'>
            {duration}
          </span>
        ) : null}
      </div>
